...
> non può **creare o cambiare autorità** di esecuzione/instradamento. Può invece alimentare sink
> locali o già vincolati (clipboard, TTS, UI, reply alla stessa chat verificata). La matrice completa
> e la propagazione del taint sono nel decision record P3 §4.
>
...
| E14 | **Geofence: aspettative realistiche.** La posizione è approssimata; attraversamenti brevi possono non produrre callback e gli eventi in background possono arrivare con **minuti** di ritardo. La [guida Android](https://developer.android.com/develop/sensors-and-location/location/geofencing#choose-the-optimal-radius-for-your-geofence) consiglia tipicamente almeno 100–150 m. | Il validator emette WARNING sotto i 100 m; l'Es. 1 con ±50 m resta accettato ma non va presentato come istantaneo. Il runtime non inventa DWELL e non genera un EXIT iniziale se la regola nasce mentre il device è già fuori. **Aggiornamento 2026-07-16:** dopo il fix sul campo `4339244`, l'audit ha trovato che il veto trattava qualunque fix come verità puntuale. Da `6b307c5` usa età e accuratezza orizzontale: smentisce il bordo soltanto quando l'intero disco d'incertezza contraddice chiaramente il framework oltre l'isteresi; fix vecchi o imprecisi non bloccano l'evento. Nota sul posizionamento: al chiuso il GNSS spesso non vede il cielo e il fix pesa su Wi-Fi/celle, dove il rumore è massimo; all'aperto e in movimento — cioè quando il geofence serve — domina il GNSS. Spegnere il Wi-Fi (Es. 1) **non** azzera il posizionamento: `Wifi scanning is always available` resta attivo. |
...
- **P3 — Fondazioni dinamiche + sensori + interattivo:** decision/dataflow e taint → state reader parametrici → sensori event-driven → runtime base senza Shizuku → loop computer-use a due tier. Il bridge lento/gratuito è baseline valida per unattended; `OpenAICompatTransport` veloce/streaming è opzionale. Piano: `2026-07-16-argus-p3-foundations-state-sensors.md`.
...
- Pubblicazione Play effettiva e gestione account hosted restano fuori P3, ma il prodotto deve avere
  profili `personal-full`/`play-core` e non introdurre dipendenze che la rendano impossibile.
